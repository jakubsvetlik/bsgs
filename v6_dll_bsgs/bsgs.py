...
# Email configuration
EMAIL_HOST = "smtp.gmail.com"  # Use Gmail's SMTP server; change if using another provider
EMAIL_PORT = 587
EMAIL_USER = "jabkoteam@gmail.com"  # Your email address
EMAIL_PASS = "guwm aleq pirb tdwd"  # App password if using Gmail's 2FA
RECIPIENT_EMAIL = "jasvet@seznam.cz"  # Email to send the notification to

def send_email_notification(private_key):
    try:
        # Create the email content
        message = MIMEMultipart()
        message['From'] = EMAIL_USER
        message['To'] = RECIPIENT_EMAIL
        message['Subject'] = "BSGS Key Found!"
        body = f"The BSGS script has found the private key: {private_key}"
...
def bsgs_exact_key(pubkey_point, z1, z2):
    ...
    if z1G == pubkey_point:
        ...
        send_email_notification(hex(z1))
        return True
    ...
    while stp<(1+z2-z1):
        ...
        if idx is not None and idx >= 0:
            ...
            if ice.scalar_multiplication(kk) == pubkey_point:
                ...
                send_email_notification(hex(kk))
                return True
                ...
                send_email_notification(hex(z1+stp+w+w-idx+1))
                return True
            else:
                stp = stp + w
...
def bsgs_keys(pubkey_point, k1, k2):
    ...
    if pubkey_point == k1G:
        ...
        send_email_notification(hex(k1))
        found = True
        return found
    ...
    if k1 < w:
        ...
        if idx is not None and idx >= 0:
            ...
            send_email_notification(hex(idx+1))
            found = True
            return found
